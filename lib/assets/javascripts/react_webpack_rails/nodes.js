...
function _mountNode(node) {
  ...
  var mountFunction = RWR.railsIntegrations[data.integrationName].mount;
  if (typeof(mountFunction) === 'function') { mountFunction(config, data.options); }
}
...
function _unmountNode(node) {
  ...
  var unmountFunction = RWR.railsIntegrations[data.integrationName].unmount;
